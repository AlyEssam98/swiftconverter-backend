...
public class CreditService {
        ...
        @Transactional
        public VerifyPurchaseResult verifyAndFulfillPurchase(String email, String sessionId) {
                // First check if already fulfilled via webhook
                if (creditPurchaseRepository.existsByTransactionId(sessionId)) {
                        log.info("Purchase already fulfilled for session: {}", sessionId);
                        CreditBalanceResponse balance = getUserCreditBalance(email);
                        return new VerifyPurchaseResult(true, balance.getAvailableCredits());
                }

                // Ask Lemon Squeezy if the order is paid
                LemonSqueezyService.VerifiedOrder order = lemonSqueezyService.verifyCheckoutSession(sessionId);
                if (order == null) {
                        log.info("No paid order found yet for session: {}", sessionId);
                        CreditBalanceResponse balance = getUserCreditBalance(email);
                        return new VerifyPurchaseResult(false, balance.getAvailableCredits());
                }

                // Fulfill the purchase now (webhook may not have arrived yet)
                try {
                        UUID userId = UUID.fromString(order.getUserId());
                        long credits = Long.parseLong(order.getCredits());

                        User user = userRepository.findById(userId).orElse(null);
                        if (user == null) {
                                log.warn("User not found for verified order: userId={}", order.getUserId());
                                CreditBalanceResponse balance = getUserCreditBalance(email);
                                return new VerifyPurchaseResult(false, balance.getAvailableCredits());
                        }

                        // Double-check after finding user (webhook might have just arrived)
                        if (creditPurchaseRepository.existsByTransactionId(order.getOrderId())) {
                                log.info("Purchase fulfilled by webhook during verify: {}", order.getOrderId());
                                CreditBalanceResponse balance = getUserCreditBalance(email);
                                return new VerifyPurchaseResult(true, balance.getAvailableCredits());
                        }

                        addPurchasedCredits(user, credits, order.getOrderId());
                        log.info("Proactively fulfilled purchase via verify endpoint: {} credits for user {}", credits, user.getEmail());

                        CreditBalanceResponse balance = getUserCreditBalance(email);
                        return new VerifyPurchaseResult(true, balance.getAvailableCredits());
                } catch (Exception e) {
                        log.error("Failed to fulfill purchase via verify for session {}: {}", sessionId, e.getMessage(), e);
                        CreditBalanceResponse balance = getUserCreditBalance(email);
                        return new VerifyPurchaseResult(false, balance.getAvailableCredits());
                }
        }
        ...
        public static class VerifyPurchaseResult {
                ...
                public VerifyPurchaseResult(boolean fulfilled, long availableCredits) {
